import 'dart:io';
import 'package:appchat/data_sources/firebase_repository.dart';
...
import '../resources/utils/strings.dart';
...
class FirebaseServices {
  ...
  Future signInWithGoogle() async {
    UserCredential? user;
    try {
      GoogleSignInAccount? signInAccount = await firebaseRepository.googleSignIn.signIn();
  ...
  Future sendMessage({required RoomChat roomChat, required Message message}) async {
    await firebaseRepository.firestore
      .collection("chats")
      .doc(roomChat.id)
      .collection("messages")
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getStreamMessage(String idRoomChat) async* {
  ...
  Future updateMessageMedias(List<String> medias, String roomChatId, String messageId) async {
    await firebaseRepository.firestore
      .collection("chats")
      .doc(roomChatId)
      .collection("messages")
  ...
  Stream<DocumentSnapshot<Map<String, dynamic>>> getStreamUser(String uid) async* {
    yield* firebaseRepository.firestore
      .collection("users")
      .doc(uid)
      .snapshots();
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getStreamRoomChat(String userId) async* {
    yield* firebaseRepository.firestore
      .collection("chats")
  ...
  Future updateUser({required String userId, required String displayName, required File? avatar}) async {
    if (avatar != null) {
